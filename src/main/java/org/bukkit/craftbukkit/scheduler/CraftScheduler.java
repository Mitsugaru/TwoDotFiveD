...
public class CraftScheduler implements BukkitScheduler, Runnable {
    ...
    void processTask(CraftTask task) {
        if (task.isSync()) {
            ...
            craftThreadManager.executeTask(task.getTask(), task.getIdNumber());
        }
    }
            ...
            try {
                ...
                while (!syncedTasks.isEmpty() && System.currentTimeMillis() <= breakTime) {
                    ...
                    try {
                        ...
                        // Bad plugin!
                        logger.log(Level.WARNING, "Task '" + task.getTaskId() + "' generated an exception", t);
                        synchronized (schedulerQueue) {
    ...
    public int scheduleSyncRepeatingTask(Runnable task, long delay, long period) {
        ...
        CraftTask newTask = new CraftTask(task, true, getCurrentTick() + delay, period);
    ...
    public int scheduleAsyncDelayedTask(Runnable task, long delay) {
        return scheduleAsyncRepeatingTask(task, delay, -1);
    }

    public int scheduleAsyncDelayedTask(Runnable task) {
        return scheduleAsyncDelayedTask(task, 0L);
    }
    ...
    public int scheduleAsyncRepeatingTask(Runnable task, long delay, long period) {
        ...
        CraftTask newTask = new CraftTask(task, false, getCurrentTick() + delay, period);
    ...
    public <T> Future<T> callSyncMethod(Callable<T> task) {
        ...
        synchronized (craftFuture) {
            int taskId = scheduleSyncDelayedTask(craftFuture);
