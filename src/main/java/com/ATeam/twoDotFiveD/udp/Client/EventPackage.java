...
public class EventPackage implements Serializable {
	...
	try {
	    // Because we cannot have a null RigidBody, this is a default rigid
	    // body
	    ...
	    final RigidBodyConstructionInfo baseInfo = new RigidBodyConstructionInfo(
		    0, new DefaultMotionState(), new BoxShape(new Vector3f(1f,
			    1f, 1f)));
		...
		String centerString = (String) data
			.get("entity.rigidbody.center");
		...
		if (shapeClass.contains("BoxShape")) {
			...
			String point = ((String) data
				.get("entity.rigidbody.collisionshape.point"
					+ i));
			point = point.replace("(", "");
			point = point.replace(",", "");
			point = point.replace(")", "");
		...
		// System.out.println("myMotionState" +
		// myMotionState.toString());
		...
		// TODO image?
		Entity body = new Entity(info, ID, new Vector3f(1f, 0f, 0f),
			null);
		...
		body.setAngularVelocity(new Vector3f(Float
			.parseFloat(angularCut[0]), Float
			.parseFloat(angularCut[1]), Float
		...
		body.setLinearVelocity(new Vector3f(Float
			.parseFloat(linearCut[0]), Float
			.parseFloat(linearCut[1]), Float
		...
		final RigidBodyConstructionInfo info = new RigidBodyConstructionInfo(
			mass, myMotionState, c, inertia);
		// TODO image?
		Entity body = new Entity(info, ID, new Vector3f(0f, 0f, 0f),
			null);
		body.setAngularFactor(((Float) data
			.get("entity.rigidbody.angularfactor")).floatValue());
		String angular = (String) data
	    ...
	    // data.get("entity.gravity.y") + " " +
	    // data.get("entity.gravity.z"));
