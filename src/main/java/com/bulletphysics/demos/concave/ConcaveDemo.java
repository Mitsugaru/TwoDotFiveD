...
public class ConcaveDemo extends DemoApplication
{
	...
	// set to false to read the BVH from disk (first run the demo once to create
	// the BVH):
		...
		for (i = 0; i < NUM_VERTS_X; i++)
		{
			for (j = 0; j < NUM_VERTS_Y; j++)
			{
				tmp.set((i - NUM_VERTS_X * 0.5f) * TRIANGLE_SIZE,
						// 0.f,
						waveheight * (float) Math.sin((float) i + offset)
	...
	public void keyboardCallback(char key, int x, int y, int modifiers)
	{
		if (key == 'g')
		{
			...
			if (animatedMesh)
			{
				staticBody.setCollisionFlags(staticBody.getCollisionFlags()
	...
	public void initPhysics()
	{
		...
		gVertices = ByteBuffer.allocateDirect(totalVerts * 3 * 4).order(
				ByteOrder.nativeOrder());
		gIndices = ByteBuffer.allocateDirect(totalTriangles * 3 * 4).order(
				ByteOrder.nativeOrder());
		...
		if (TEST_SERIALIZATION)
		{
			...
			{
				trimeshShape = new BvhTriangleMeshShape(indexVertexArrays,
		...
		// Win32ThreadSupport* threadSupport = new
		// Win32ThreadSupport(Win32ThreadSupport::Win32ThreadConstructionInfo(
		// "collision",
		...
		// ///pthreads version
		// ///you can hook it up to your custom task scheduler by deriving from
		// btThreadSupportInterface
		// #endif
		//
		// m_dispatcher = new
		// SpuGatheringCollisionDispatcher(threadSupport,maxNumOutstandingTasks,m_collisionConfiguration);
		// #else
		...
		else
		{
			...
			CollisionShape cylinderShape = new CylinderShapeX(new Vector3f(4,
					1, 1));
			...
			((CompoundShape) colShape).addChildShape(localTransform,
					cylinderShape);
		}
		...
		
		{
			for (i = 0; i < 10; i++)
			{
				...
				// btCapsuleShape(0.5,2.0);//boxShape = new btSphereShape(1.f);
	...
	public void clientMoveAndDisplay()
	{
		...
		if (animatedMesh)
		{
			...
			dynamicsWorld
					...
					.cleanProxyFromPairs(staticBody.getBroadphaseHandle(),
	 ...
	 */
	private static float calculateCombinedFriction(float friction0,
			float friction1)
	{
	...
	private static float calculateCombinedRestitution(float restitution0,
			float restitution1)
	{
		return restitution0 * restitution1;
	}
	
	private static class CustomMaterialCombinerCallback extends
			ContactAddedCallback
	{
				...
				int index1)
		{
			...
			cp.combinedFriction = calculateCombinedFriction(friction0,
					friction1);
			cp.combinedRestitution = calculateCombinedRestitution(restitution0,
					restitution1);
