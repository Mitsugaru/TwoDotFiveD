...
public abstract class DemoApplication
{
	...
	public static CollisionShape[]	gShapePtr				= new CollisionShape[maxNumObjects];	// 1
																									// rigidbody
																									// has
																									// 1
																									// shape
	...
	public static RigidBody			pickedBody				= null;								// for
																									// deactivation
																									// state
	...
	protected final Vector3f		cameraPosition			= new Vector3f(0f,
																	0f, 0f);
	protected final Vector3f		cameraTargetPosition	= new Vector3f(0f,
																	0f, 0f);						// look
																									// at
																									
	protected float					scaleBottom				= 0.5f;
	protected float					scaleFactor				= 2f;
	protected final Vector3f		cameraUp				= new Vector3f(0f,
																	1f, 0f);
			...
			int modifiers)
	{
		...
		switch (key)
		{
			...
			case 'm':
				...
				{
					debugMode = debugMode
							& (~DebugDrawModes.ENABLE_SAT_COMPARISON);
				}
				else
				{
			...
			case 'n':
				...
				{
					debugMode = debugMode
							& (~DebugDrawModes.DISABLE_BULLET_LCP);
				}
				else
				{
			...
			case 'y':
				...
				{
					debugMode = debugMode
							& (~DebugDrawModes.DRAW_FEATURES_TEXT);
				}
				else
				{
			...
			case '5':
			{
						...
						.getCollisionObjectArray())
				{
					if (o instanceof RigidBody)
					{
						...
						rb.setGravity(new Vector3f(0f, -10f, 0f));
			...
			{
				/*
				 * for(final CollisionObject o :
				 * dynamicsWorld.getCollisionObjectArray()) { if(o instanceof
				 * RigidBody) { final RigidBody rb = (RigidBody) o;
				 * if(rb.isActive()) { //TODO Figure out how to remove last
				 * object without crashing //TODO Figure out why it doesn't
		...
		if (getDynamicsWorld() != null
				&& getDynamicsWorld().getDebugDrawer() != null)
		{
	...
	public void specialKeyboard(int key, int x, int y, int modifiers)
	{
		switch (key)
		{
			...
			case Keyboard.KEY_END:
			{
				...
				if (numObj != 0)
				{
					CollisionObject obj = getDynamicsWorld()
							.getCollisionObjectArray().getQuick(numObj - 1);
	...
	public synchronized void shootBox(Vector3f destination)
	{
		if (dynamicsWorld != null)
		{
			...
			if (shapeType.equals("BOX"))
			{
				...
				// btConvexShape* childShape = new
				// btBoxShape(btVector3(1.f,1.f,1.f));
			...
			case 1:
			{
				if (state == 0)
				{
					...
					if (dynamicsWorld != null)
					{
						...
						dynamicsWorld.rayTest(cameraPosition, rayTo,
								rayCallback);
			...
			case 0:
			{
				...
				else
				{
					...
					if (pickConstraint != null && dynamicsWorld != null)
					{
						...
						pickConstraint = null;
						pickedBody
								.forceActivationState(CollisionObject.ACTIVE_TAG);
	...
	public synchronized RigidBody localCreateRigidBody(float mass,
	...
	protected float showProfileInfo(float xOffset, float yStart, float yIncr)
	{
		...
		
		{
			...
			double parent_time = profileIterator.isRoot() ? time_since_reset
			...
			{
				double current_total_time = profileIterator
						.getCurrentTotalTime();
	...
	public synchronized void renderme()
	{
		...
		if (dynamicsWorld != null)
		{
			...
			if ((debugMode & DebugDrawModes.NO_HELP_TEXT) == 0)
			{
				...
				// for (iter = btProfiler::mProfileBlocks.begin(); iter !=
				// btProfiler::mProfileBlocks.end(); ++iter)
				// {
				// char blockTime[128];
				// sprintf(blockTime,
				// "%s: %lf",&((*iter).first[0]),btProfiler::getBlockTime((*iter).first,
				...
				// bool useBulletLCP = !(getDebugMode() &
